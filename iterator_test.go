...
func TestName(t *testing.T) {
	testCases := []struct {
		...
		{input: "112.65.183.122/255.255.255.248", want: "1.1.1.3"},
	...
	for _, v := range testCases {
		...
		t.Logf("Size %d", it.GetTotalNum())
		index := it.GetTotalNum() - 1
		if index > 2 {
			index -= 2
		}

		index2 := it.GetTotalNum() - 1
		if index2 > 3 {
			index2 -= 3
		}
	...
	// 简单的获取IP序列
	ipSet, err := GenIpSet("112.65.183.122/255.255.255.248")
	if err != nil {
		t.Fatal(err)
	}
